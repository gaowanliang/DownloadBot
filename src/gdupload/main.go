...
func GetAllFile(pathname string, folderIDList []string, srv *drive.Service) error {
	...
	for _, fi := range rd {
		if fi.IsDir() {
			...
			err := GetAllFile(fullDir, tempFolderIDList, srv)
			if err != nil {
			...
			driveFile, err := srv.Files.Create(&drive.File{Name: fi.Name(), Parents: tempFolderIDList}).Media(f).Do()
			if err != nil {
